...
private:
  ...
  time_t write_timeout_usec_;
        ...
        socket_t create_socket(const char *host, int port, int address_family,
            ...
            auto service = stdpatch::to_string(port);
...
private:
  ...
  BrotliDecoderState *decoder_s = nullptr;
        ...
        inline uint64_t get_header_value<uint64_t>(const Headers &headers,
            ...
            std::advance(it, static_cast<ssize_t>(id));
            if (it != rng.second) {
                return stdpatch::strtoull(it->second.data(), nullptr, 10);
            }
            return def;
        }
            ...
            if (std::regex_match(s, m, re_first_range)) {
                ...
                split(&s[pos], &s[pos + len], ',', [&](const char *b, const char *e) {
                    ...
                    if (std::regex_match(b, e, cm, re_another_range)) {
                        ssize_t first = -1;
                        if (!cm.str(1).empty()) {
                            first = static_cast<ssize_t>(stdpatch::stoll(cm.str(1)));
                        }

                        ssize_t last = -1;
                        if (!cm.str(2).empty()) {
                            last = static_cast<ssize_t>(stdpatch::stoll(cm.str(2)));
                        }
        ...
        inline std::string make_content_range_header_field(size_t offset, size_t length,
            ...
            field += stdpatch::to_string(offset);
            field += "-";
            field += stdpatch::to_string(offset + length - 1);
            field += "/";
            field += stdpatch::to_string(content_length);
            return field;
        }
...
// Header utilities
    inline std::pair<std::string, std::string> make_range_header(Ranges ranges) {
        ...
        for (auto r : ranges) {
            if (i != 0) { field += ", "; }
            if (r.first != -1) { field += stdpatch::to_string(r.first); }
            field += '-';
            if (r.second != -1) { field += stdpatch::to_string(r.second); }
            i++;
        }
...
// HTTP server implementation
    ...
    inline void Server::apply_ranges(const Request &req, Response &res,
        ...
        if (res.body.empty()) {
            if (res.content_length_ > 0) {
                ...
                res.set_header("Content-Length", stdpatch::to_string(length));
            } else {
        ...
        content_encoding = "br";
            ...
            auto length = stdpatch::to_string(res.body.size());
    ...
    Server::process_request(Stream &strm, bool close_connection,
        ...
        req.set_header("REMOTE_PORT", stdpatch::to_string(req.remote_port));
...
// HTTP client implementation
    ...
    // : (Error::Success), host_(host), port_(port),
            : host_(host), port_(port),
              host_and_port_(host_ + ":" + stdpatch::to_string(port_)),
  ...
  proxy_digest_auth_password_ = rhs.proxy_digest_auth_password_;
    ...
    inline bool ClientImpl::read_response_line(Stream &strm, const Request &req,
        ...
        res.status = stdpatch::stoi(std::string(m[2]));
    ...
    inline bool ClientImpl::redirect(Request &req, Response &res, Error &error) {
        ...
        if (!port_str.empty()) {
            next_port = stdpatch::stoi(port_str);
    ...
    inline bool ClientImpl::write_request(Stream &strm, Request &req,
        ...
        if (req.body.empty()) {
            if (req.content_provider_) {
                if (!req.is_chunked_content_provider_) {
                    if (!req.has_header("Content-Length")) {
                        auto length = stdpatch::to_string(req.content_length_);
            ...
            if (!req.has_header("Content-Length")) {
                auto length = stdpatch::to_string(req.body.size());
                req.headers.emplace("Content-Length", length);
            }
        }
...
// Universal client implementation
    ...
    inline Client::Client(const char *scheme_host_port,
        ...
        if (std::regex_match(scheme_host_port, m, re)) {
            ...
            auto port_str = m[3].str();
            auto port = !port_str.empty() ? stdpatch::stoi(port_str) : (is_ssl ? 443 : 80);
